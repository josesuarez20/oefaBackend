...
@Component("documentoFacade")
public class DocumentoFacadeImpl extends FacadeBase implements DocumentoFacade {
	private CoreLogger logger = LoggerFactory.getLogger(DocumentoFacadeImpl.class);
	@Autowired
			...
			if (GenericUtil.isNotEmpty(files)) {
				for (MultipartFile multipartFile : files) {
					...
					if (multipartFile.getContentType().equalsIgnoreCase("application/pdf")
						...
						documentoDTO.setUbicacionDocumento(parts[1].replace("|", "/"));// ruta del archivo
